...
/*   Updated: 2025/01/10 16:52:07 by mmaria-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int TestPart1(int testNumber)
{
	...
	try
	{
		...
		SlabAllocator<int, Nginx_MemoryPool> 					
													alloc(pool, poolsize);
		
		std::list<int, SlabAllocator<int, Nginx_MemoryPool> > 
													list1(alloc);
		...
		for (std::list<int, SlabAllocator<int, Nginx_MemoryPool> >::iterator it = list1.begin(); it != list1.end(); ++it)
		{
			size_t nodeAddress;
	...
	try
	{
		...
		*/
		...
		std::list<int, SlabAllocator<int, Nginx_MemoryPool> > 
													list1(SlabAllocator<int, Nginx_MemoryPool>(pool, poolsize));
		...
		std::list<int, SlabAllocator<int, Nginx_MemoryPool> > 
													list2(SlabAllocator<int, Nginx_MemoryPool>(pool, poolsize));
		...
		for (std::list<int, SlabAllocator<int, Nginx_MemoryPool> >::iterator it = list2.begin(); it != list2.end(); ++it)
		{
			size_t nodeAddress;
			nodeAddress = (size_t)&(*it);
