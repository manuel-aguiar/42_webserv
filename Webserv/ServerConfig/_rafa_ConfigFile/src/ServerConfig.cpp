...
{
	m_keys["host"]				= &ServerConfig::setHost;
	m_keys["port"]				= &ServerConfig::addPort;
	m_keys["server_names"]		= &ServerConfig::addServerName;
	m_keys["client_body_size"]	= &ServerConfig::setClientBodySize;
	m_keys["client_header_size"]	= &ServerConfig::setClientHeaderSize;
	m_keys["max_connections"]	= &ServerConfig::setMaxConnections;
	m_keys["max_concurrent_cgi"]	= &ServerConfig::setMaxConcurrentCGI;
	m_keys["root"]				= &ServerConfig::setRootPath;
	m_keys["error_pages"]		= &ServerConfig::addErrorPage;
...
{
	(void)flag;
	if (!is_number(value) || (m_config["port"].find(value) != m_config["port"].end()))
		return (0);
...
void	ServerConfig::set_defaults(const int &flag)
{
	...
	if (m_config["port"].empty())
		m_config["port"].insert(DEFAULT_PORT);
	try {
