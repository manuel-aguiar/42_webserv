...
namespace Cgi
{
    ...
    Cgi::Request*	Module::acquireRequest()
    {
        return (ImplModule::acquireRequest());
    }

    void			Module::enqueueRequest(Cgi::Request& data, bool isCalledFromEventLoop)
    {
        ImplModule::enqueueRequest(data, isCalledFromEventLoop);
    }

    void			Module::modifyRequest(Cgi::Request& data, bool isCalledFromEventLoop, Options::Mask newOptions)
    {
        ImplModule::modifyRequest(data, isCalledFromEventLoop, newOptions);
    }

    void			Module::finishRequest(Cgi::Request& data, bool isCalledFromEventLoop)
    {
    ...
    void			Module::addInterpreter(const InterpExtension& extension, const InterpPath& path)
    {
        ImplModule::addInterpreter(extension, path);
    }

    void			Module::removeInterpreter(const InterpExtension& extension)
    {
        ImplModule::removeInterpreter(extension);
    }

    size_t          Module::getBusyWorkerCount() const
    {
        return (ImplModule::getBusyWorkerCount());
    }

    size_t			Module::getQueueSize() const
    {
