...
Response::mf_validateHeaders()
{
	...
	for (size_t i = 1; i < m_headers.size(); ++i)
	{
		if (m_headers[i - 1].key == m_headers[i].key)
		{
			...
			return (mf_setStatus(Response::FAIL, CGI_FAILURE));
		}
	}
...
Response::mf_parseHeaders(BufferView& view)
{
	...
	if (pos == 0 && m_state != Response::START)
	{
	...
	{
		if (m_statusCode != -1)
			return (mf_setStatus(Response::FAIL, CGI_FAILURE)); // doubled status header
		m_statusCode = std::atoi(value.data());
		if (m_statusCode != CGI_SUCCESS)
			return (mf_setStatus(Response::FAIL, m_statusCode));
	}
	else	
...
Response::Status	Response::parse(BaseBuffer& buffer)
{
	...
	if (view.size() == 0)
	{
		if (m_state == Response::BODY)
		{
			...
			return (mf_setStatus(Response::FAIL, CGI_FAILURE)); // still in headers and received EOF
	}
	...
	while (view.size() > 0)
	{
		switch ((int)m_state)
		{
			case Response::START: //allow to fall to headers
...
Response::Status
Response::mf_setStatus(const Response::Status status, const int statusCode)
{
