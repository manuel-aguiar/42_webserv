...
namespace Cgi
{
	...
	void	Cgi::Module::Worker::mf_KillWaitChild()
	{
		if (m_pid == -1)
			return ;
		::kill(m_pid, SIGKILL);
	...
	void	Cgi::Module::Worker::mf_childFailure()
	{
		std::string errorMsg;
	...
	void	Cgi::Module::Worker::mf_waitChild()
	{
		int status;
	...
	void 	Cgi::Module::Worker::mf_closeFd(Ws::fd& fd)
	{
		if (fd != -1 && ::close(fd) == -1)
