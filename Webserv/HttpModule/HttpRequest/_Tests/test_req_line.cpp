...
void testRequestLineParsing() {
...

    {
        ...
        try {
            ...
            if (status != Http::Status::OK)
                throw std::runtime_error("Expected status OK, got: " + to_string(status));
...

    {
        ...
        try {
            ...
            if (status != Http::Status::METHOD_NOT_ALLOWED)
                throw std::runtime_error("Expected status METHOD_NOT_ALLOWED, got: " + to_string(status));
...

    {
        ...
        try {
            ...
            printRequest("GET", "(URI length: " + to_string(long_uri.length()) + ")", "HTTP/1.1", "Host: localhost");

            HttpRequest request;
            int status = request.parse(raw_request);

            if (status != Http::Status::URI_TOO_LONG)
                throw std::runtime_error("Expected status URI_TOO_LONG, got: " + to_string(status));
...

    {
        ...
        try {
            ...
            if (status != Http::Status::BAD_REQUEST)
                throw std::runtime_error("Expected status BAD_REQUEST, got: " + to_string(status));
