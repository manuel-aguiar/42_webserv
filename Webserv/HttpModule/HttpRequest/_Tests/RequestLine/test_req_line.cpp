...
void validRequestLineTests(int &testNumber)
{
    ...
    TEST_INTRO(testNumber++);
    {
        ...
        try
        {
            ...
            EXPECT_EQUAL(httpRequest.getMethod(), "GET", "Method should be GET");
            EXPECT_EQUAL(httpRequest.getUri(), "/index.html", "URI should match");
            EXPECT_EQUAL(httpRequest.getHttpVersion(), "HTTP/1.1", "Version should match");
    ...
    TEST_INTRO(testNumber++);
    {
        ...
        {
            EXPECT_EQUAL(httpRequest.parse(requestData), (int)Http::Status::OK, "Should pass");
            const std::map<std::string, std::string>& components = httpRequest.getUriComponents();
...
void invalidRequestLineTests(int &testNumber)
{
    ...
    TEST_INTRO(testNumber++);
    {
        ...
        try
        {
            EXPECT_EQUAL(httpRequest.parse(requestData), (int)Http::Status::METHOD_NOT_ALLOWED, "Should fail");
    ...
    TEST_INTRO(testNumber++);
    {
        ...
        try
        {
            EXPECT_EQUAL(httpRequest.parse(requestData), (int)Http::Status::URI_TOO_LONG, "Should fail");
